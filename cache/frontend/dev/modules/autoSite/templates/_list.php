    ...
    <table cellspacing="0" class="a-admin-list-table">
      <thead>
        <tr>
					          	...
					          	<?php include_partial('Site/list_th_tabular', array('sort' => $sort, 'helper' => $helper)) ?>
      ...
      <tbody>
        <?php $n=1; $total = $pager->getNbResults(); foreach ($pager->getResults() as $i => $site): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr class="a-admin-row <?php echo $odd ?> <?php echo ($n == $total)? 'last':'' ?> <?php echo ($n == 1)? 'first':'' ?>">
						            	<?php include_partial('Site/list_td_batch_actions', array('site' => $site, 'helper' => $helper)) ?>
						            	<?php include_partial('Site/list_td_tabular', array('site' => $site, 'helper' => $helper)) ?>
