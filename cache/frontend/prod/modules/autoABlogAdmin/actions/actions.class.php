...
require_once(dirname(__FILE__).'/../lib/BaseABlogAdminGeneratorConfiguration.class.php');
require_once(dirname(__FILE__).'/../lib/BaseABlogAdminGeneratorHelper.class.php');
 ...
 */
class autoABlogAdminActions extends sfActions
{
  public function preExecute()
  {
    $this->configuration = new aBlogAdminGeneratorConfiguration();
    ...
    $this->helper = new aBlogAdminGeneratorHelper();
  ...
  public function executeBatch(sfWebRequest $request)
  {
    ...
    $validator = new sfValidatorDoctrineChoice(array('multiple' => true,  'model' => 'aBlogPost'));
    try
    {
      // validate ids
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      ...
      $a_blog_post = $form->save();

      $this->dispatcher->notify(new sfEvent($this, 'admin.save_object', array('object' => $a_blog_post)));

      if ($request->hasParameter('_save_and_add'))
      {
        $this->getUser()->setFlash('notice', $this->getUser()->getFlash('notice').' You can add another one below.');

        $this->redirect('@a_blog_admin_new');
      }
      else
      {
        $this->redirect('@a_blog_admin_edit?id='.$a_blog_post->getId());
      }
    }
    else
    {
  ...
  {
    return $this->getUser()->getAttribute('aBlogAdmin.filters', $this->configuration->getFilterDefaults(), 'admin_module');
  }

  protected function setFilters(array $filters)
  {
    return $this->getUser()->setAttribute('aBlogAdmin.filters', $filters, 'admin_module');
  }

  protected function getPager()
  {
    $pager = $this->configuration->getPager('aBlogPost');
  ...
  {
    $this->getUser()->setAttribute('aBlogAdmin.page', $page, 'admin_module');
  }

  protected function getPage()
  {
    return $this->getUser()->getAttribute('aBlogAdmin.page', 1, 'admin_module');
  }
  ...
  {
    if (!is_null($sort = $this->getUser()->getAttribute('aBlogAdmin.sort', null, 'admin_module')))
    {
      return $sort;
    }
    ...
    return $this->getUser()->getAttribute('aBlogAdmin.sort', null, 'admin_module');
  }
  ...
  protected function setSort(array $sort)
  {
    ...
    $this->getUser()->setAttribute('aBlogAdmin.sort', $sort, 'admin_module');
  }
}
